...
def main():
    ...
    if not model_path.exists():
        ...
        print("  Run: python gate/train_gate.py --fit-final")
        sys.exit(1)

    table_path = ROOT / gate_cfg["table_path"]
    if not table_path.exists():
        print(f"[ERROR] Table not found: {table_path}")
        print("  Run: python gate/build_gate_training_table.py")
        sys.exit(1)
    ...
    if proba.shape[1] == 1:
        p_use_s2 = np.zeros(len(rows))  # model only saw one class → predict all 0 (S1)
    else:
        p_use_s2 = proba[:, 1]
