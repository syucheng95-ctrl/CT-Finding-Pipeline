...
OUT_DIR = ROOT / "gate" / "outputs"


def _load_config():
    """Load config.yaml from upload/ root."""
    with open(ROOT / "config.yaml", encoding="utf-8") as f:
        config = yaml.safe_load(f)
    # Resolve ./ and ../ paths relative to ROOT
    def _resolve(obj):
        if isinstance(obj, dict): return {k: _resolve(v) for k, v in obj.items()}
        if isinstance(obj, list): return [_resolve(v) for v in obj]
...
def main():
    ...
    config = _load_config()
    manifest_path = Path(config.get("manifests", {}).get("default", ROOT / "data" / "all_100.jsonl"))
