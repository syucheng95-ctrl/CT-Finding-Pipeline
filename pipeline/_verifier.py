...
def _setup_imports(upload_dir: Path) -> None:
    """Ensure voxtell and stage1 modules are importable from upload/workspace/."""
    vendor = upload_dir / "workspace" / "voxtell"
    stage1_verify = upload_dir / "workspace" / "stage1"
    for p in [str(vendor), str(stage1_verify)]:
        if p not in sys.path:
            sys.path.insert(0, p)


def _build_predictor(model_dir: str, device: torch.device, text_model: str):
    """Build a DoRAEvalPredictor (or base VoxTellPredictor) for inference."""
    from eval import DoRAEvalPredictor
    return DoRAEvalPredictor(model_dir=model_dir, device=device, text_encoding_model=text_model)
