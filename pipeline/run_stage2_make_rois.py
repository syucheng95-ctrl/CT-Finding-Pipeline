...
Margin logic matches training (recrop_v3_rois.py):
...
def load_mask_component_bboxes(mask_path, proposal_bbox_hwd, min_fg=MIN_MASK_FG):
    ...
    Training ROIs are component-level crops. Keeping VoxTell coarse-mask ROIs
...
def main():
    ...
    for vrow in verified_rows:
        ...
        if not label_path.exists():
            continue
        resolved_rows.append((str(source_ct_path), str(label_path), vrow))
    ...
    for ct_key, label_key, vrow in resolved_rows:
        ...
        for lesion_box, bbox_source, _comp_size in finding_lesions:
            ...
            # Apply margin (aligned with training)
            ...
            roi_rows.append({
                ...
                "roi_mask_path": f"roi_masks/{comp_id}_mask.nii.gz",
