"""Stratified evaluation + holdout evaluation for STU-Net checkpoint.

Modes:
  full-roi       Process each full ROI once after pad-to-32, no crop.
  fixed-patch    Match training validation: center crop/pad to per-group patch.
...
from dataset import GROUP_ORDER, ReXStage2ROIPatchDataset, get_group, load_jsonl
...
def pad_to_multiple_centered(volume: np.ndarray, multiple: int, cval: float):
    """Pad symmetrically so the original ROI stays centered (like training center-crop/pad but pad-only, no crop)."""
    h, w, d = volume.shape
    target_shape = (
...
def eval_full_roi(
    ...
    for idx, row in enumerate(rows, start=1):
        if progress_interval > 0 and idx % progress_interval == 0:
            print(f"    {split}: {idx}/{len(rows)}", flush=True)
...
def eval_fixed_patch(model, manifest_path, data_root, split, device, group_patch_shapes, group_batches, progress_interval=0):
    """Evaluate with training-style per-group center crop/pad patches."""
    group_metrics = defaultdict(lambda: {"dice": [], "recall": [], "precision": []})
    all_metrics = []
        ...
        with torch.no_grad():
            for batch in loader:
                ...
                if progress_interval > 0 and seen % progress_interval == 0:
                    print(f"    {split}/{group}: {seen}/{len(ds)}", flush=True)
...
def main():
    ...
    p.add_argument("--splits", nargs="+", default=["val", "holdout"], choices=["train", "val", "holdout"],
    ...
    if "val_dice" in ckpt:
        print(f"  saved val_dice: {ckpt['val_dice']:.4f}")
    ...
    for split in args.splits:
        if args.mode == "full-roi":
            ...
            group_metrics, overall, skipped = eval_fixed_patch(
                model, args.manifest, args.data_root, split, device, group_patch_shapes, group_batches, args.progress_interval
            )
        else:
        ...
        print_results(split.upper(), group_metrics, overall, skipped)
