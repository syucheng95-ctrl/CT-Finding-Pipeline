"""Sliding-window evaluation for a trained experiment checkpoint."""
...
from config import TRAIN_CFG, EXP_CONFIGS, build_voxtell_from_checkpoint, apply_for_exp
...
class DoRAEvalPredictor(VoxTellPredictor):
    def __init__(
        self,
        model_dir: str,
...
def main():
    ...
    # Build model with DoRA applied
    r_val = exp_cfg["r"] if exp_cfg["r"] > 0 else 8
    ...
    ckpt_exp = ckpt.get("exp")
    if ckpt_exp is not None and ckpt_exp != args.exp:
        raise ValueError(f"Checkpoint exp={ckpt_exp!r} does not match --exp {args.exp!r}")
    model.load_state_dict(ckpt["model_state_dict"], strict=True)
    model = model.to(device).eval()

    # Create predictor with DoRA network
    local_text_model = Path(__file__).resolve().parent.parent / "models" / "Qwen3-Embedding-4B"
    text_model_name = args.text_model or (str(local_text_model) if local_text_model.exists() else "Qwen/Qwen3-Embedding-4B")
    ...
    res = {
        "exp": args.exp, "checkpoint": str(args.checkpoint),
