...
from router_policy import labels_for_category
...
def resolve_path(path: str) -> Path:
    ...
    if p.is_absolute():
        return p
    for base in (Path.cwd(), SCRIPT_DIR, SCRIPT_DIR.parent, SCRIPT_DIR.parents[1]):
...
def build_split(split: str, rows, tokenizer, model, qwen_cfg, artifact_dir: Path, device):
    ...
    out_path = artifact_dir / "embeddings" / f"{split}.pt"
    out_path.parent.mkdir(parents=True, exist_ok=True)
    torch.save(out, out_path)
    print(f"saved {split}: {out_path} shape={tuple(embeddings.shape)}")


def main():
    parser = argparse.ArgumentParser()
    parser.add_argument("--config", default="configs/router_qwen.json")
    parser.add_argument("--splits", nargs="+", default=["train", "val", "holdout"])
    ...
    for split in args.splits:
        rows = read_jsonl(data_dir / f"router_{split}.jsonl")
        build_split(split, rows, tokenizer, model, qwen_cfg, artifact_dir, device)
