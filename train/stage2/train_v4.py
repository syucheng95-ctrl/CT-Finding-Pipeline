...
CROP_MODES = {
    "small": "center",
    "medium": "center",
    "large": "foreground_random",
    "xlarge": "foreground_random",
}

FOREGROUND_CROP_PROBS = {
    "small": 0.0,
    "medium": 0.0,
    "large": 0.7,
    "xlarge": 0.7,
...
def build_train_loaders(manifest: Path, data_root: Path, group_steps: dict, num_workers: int):
    ...
    for group in GROUP_ORDER:
        ...
        ds = ReXStage2ROIPatchDataset(
            ...
            foreground_crop_prob=FOREGROUND_CROP_PROBS[group],
        )

        n_samples = steps * batch
...
def validate_manifest_files(manifest: Path, data_root: Path):
    ...
    bad = []
    split_counts = {
        "train": {g: 0 for g in GROUP_ORDER},
        "val": {g: 0 for g in GROUP_ORDER},
    }
    ...
    for row in rows:
        ...
        split = row["split"]
        group = row.get("group")
        if group not in split_counts[split]:
    ...
    empty_train_groups = [g for g, n in split_counts["train"].items() if n == 0]
    empty_val_groups = [g for g, n in split_counts["val"].items() if n == 0]
    if bad or empty_train_groups or empty_val_groups:
        details = "\n".join(
            f"  {sample_id} {key} {reason}: {path}"
            for sample_id, key, path, reason in bad
        )
        if empty_train_groups:
            details += "\n  empty train groups: " + ", ".join(empty_train_groups)
        if empty_val_groups:
            details += "\n  empty val groups: " + ", ".join(empty_val_groups)
        raise RuntimeError(
...
def apply_runtime_group_config(args):
    ...
    if args.steps_per_group is not None:
        for group in GROUP_ORDER:
            GROUP_STEPS[group] = int(args.steps_per_group)
    else:
        GROUP_STEPS["small"] = int(args.small_steps)
        GROUP_STEPS["medium"] = int(args.medium_steps)
        GROUP_STEPS["large"] = int(args.large_steps)
        GROUP_STEPS["xlarge"] = int(args.xlarge_steps)

    FOREGROUND_CROP_PROBS["large"] = float(args.large_foreground_crop_prob)
    FOREGROUND_CROP_PROBS["xlarge"] = float(args.xlarge_foreground_crop_prob)

    for group in GROUP_ORDER:
        if GROUP_STEPS[group] <= 0:
            raise ValueError(f"{group} steps must be > 0")
        if GROUP_CONFIGS[group]["batch"] <= 0:
            raise ValueError(f"{group} batch must be > 0")
    for group, prob in FOREGROUND_CROP_PROBS.items():
        if prob < 0.0 or prob > 1.0:
            raise ValueError(f"{group} foreground crop probability must be in [0, 1]")
...
def main():
    ...
    parser.add_argument("--steps-per-group", type=int, default=None,
                        help="Override all per-group step counts with one value.")
    parser.add_argument("--small-steps", type=int, default=200)
    parser.add_argument("--medium-steps", type=int, default=250)
    parser.add_argument("--large-steps", type=int, default=600)
    parser.add_argument("--xlarge-steps", type=int, default=700)
    ...
    parser.add_argument("--save-dir", type=Path, default=Path("outputs/stage2_v4"))
    ...
    parser.add_argument("--scheduler-t0", type=int, default=12)
    parser.add_argument("--large-foreground-crop-prob", type=float, default=0.7)
    parser.add_argument("--xlarge-foreground-crop-prob", type=float, default=0.7)
    parser.add_argument("--small-loss-weight", type=float, default=1.0,
                        help="Loss weight for small group (default: 1.0)")
    parser.add_argument("--medium-loss-weight", type=float, default=1.0,
                        help="Loss weight for medium group (default: 1.0)")
    parser.add_argument("--large-loss-weight", type=float, default=1.0,
                        help="Loss weight for large group (default: 1.0)")
    parser.add_argument("--xlarge-loss-weight", type=float, default=1.0,
                        help="Loss weight for xlarge group (default: 1.0)")
    ...
    parser.add_argument("--skip-val", action="store_true", help="Skip epoch validation; useful for smoke tests.")
    ...
    print("=" * 60)
    # Build per-group loss weights
    LOSS_WEIGHTS = {
        "small": float(args.small_loss_weight),
        "medium": float(args.medium_loss_weight),
        "large": float(args.large_loss_weight),
        "xlarge": float(args.xlarge_loss_weight),
    }
    if any(v <= 0 for v in LOSS_WEIGHTS.values()):
        raise ValueError(f"Loss weights must be > 0, got {LOSS_WEIGHTS}")

    print("Multi-scale training V4-light (foreground crop + per-group steps)")
    print(f"  total steps/epoch: {sum(GROUP_STEPS.values())}")
    for g in GROUP_ORDER:
        ...
        lw = LOSS_WEIGHTS[g]
        steps = GROUP_STEPS[g]
        fg_prob = FOREGROUND_CROP_PROBS[g]
        print(
            f"  {g:>8}: patch={shape[0]}x{shape[1]}x{shape[2]}  batch={batch}  "
            f"steps={steps}  crop={cm}  fg_prob={fg_prob}  loss_w={lw}"
        )
    print("=" * 60)
    ...
    val_loaders = {} if args.skip_val else build_val_loaders(args.manifest, args.data_root, args.num_workers)
    val_n = 0 if args.skip_val else sum(
    ...
    for epoch in epoch_bar:
        ...
        train_loaders = build_train_loaders(
            args.manifest, args.data_root, GROUP_STEPS, args.num_workers
        )
        loader_iters = {g: iter(train_loaders[g]) for g in GROUP_ORDER}
        epoch_schedule = [
            group
            for group in GROUP_ORDER
            for _ in range(GROUP_STEPS[group])
        ]
        random.shuffle(epoch_schedule)
        ...
        model.train()
        step_bar = tqdm(
            enumerate(epoch_schedule, start=1),
        ...
        for step, group in step_bar:
            ...
            with torch.autocast(device_type=device.type, dtype=torch.float16, enabled=(device.type == "cuda")):
                ...
                dice_l = dice_loss_from_logits(logits, y)
                raw_loss = ce + dice_l
                loss = raw_loss * LOSS_WEIGHTS[group]
            ...
            epoch_loss[group].append(float(raw_loss.item()))
            with torch.no_grad():
                m = batch_segmentation_metrics(logits, y)
            epoch_dice[group].append(m["dice"])
            step_postfix[f"{group[0]}_loss"] = f"{raw_loss.item():.3f}"
            ...
            if global_step % args.log_interval == 0:
                ...
                print(
                    ...
                    f"loss={raw_loss.item():.4f} weighted_loss={loss.item():.4f} dice={m['dice']:.4f} "
        ...
        if args.skip_val:
            group_metrics = {}
            val_overall = {"loss": 0.0, "dice": -1.0, "recall": 0.0, "precision": 0.0}
        else:
        ...
        checkpoint_payload = {
            ...
            "foreground_crop_probs": FOREGROUND_CROP_PROBS,
